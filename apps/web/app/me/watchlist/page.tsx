...
import { BookmarkX, ChevronLeft, ChevronRight, Trash2 } from "lucide-react";
...
import { useRemoveWatchlist, useWatchlist } from "@/lib/me";
...
export default function WatchlistPage() {
    ...
    return (
      <div className="flex flex-col items-center gap-3 rounded-xl border border-border bg-card p-12 text-center">
        <p className="text-sm text-muted-foreground">
          Bạn cần đăng nhập để xem danh sách muốn xem.
        </p>
        <Button asChild>
          <Link href="/login?next=/me/watchlist">Đăng nhập</Link>
        </Button>
      </div>
    );
  }
  ...
  return (
    <section className="space-y-4">
      <h1 className="text-2xl font-bold">Muốn xem</h1>
      {!data || data.items.length === 0 ? (
        <div className="flex flex-col items-center gap-3 rounded-xl border border-border bg-card p-12 text-center">
          <BookmarkX className="size-10 text-muted-foreground" />
          <p className="text-sm text-muted-foreground">
            Danh sách trống. Nhấn “Muốn xem” ở trang chi tiết để để dành phim
            xem sau nhé — bắt đầu xem là phim tự rời khỏi đây.
          </p>
                ...
                <button
                  onClick={() =>
                    remove.mutate(f.movie_slug, {
                      onSuccess: () =>
                        toast("Đã xóa khỏi danh sách muốn xem.", "success"),
                      onError: () => toast("Xóa thất bại. Thử lại nhé.", "error"),
                    })
                  }
                  aria-label={`Xóa ${f.movie_name} khỏi danh sách muốn xem`}
