...
async def add(
    db: AsyncSession, profile_id: uuid.UUID, data: FavoriteAdd
) -> tuple[Favorite, bool]:
    stmt = select(Favorite).where(
        Favorite.profile_id == profile_id, Favorite.movie_slug == data.movie_slug
    )
    existing = (await db.execute(stmt)).scalar_one_or_none()
    if existing is not None:
        return existing, False
    row = Favorite(profile_id=profile_id, **data.model_dump())
    db.add(row)
    await db.commit()
...
async def remove(db: AsyncSession, profile_id: uuid.UUID, movie_slug: str) -> None:
    await db.execute(
        delete(Favorite).where(
            Favorite.profile_id == profile_id, Favorite.movie_slug == movie_slug
        )
    )
    await db.commit()
